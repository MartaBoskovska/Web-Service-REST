...
import web.agence.repositories.OffreAgenceRepository2;
...
@RestController
public class AgenceController2 {
	
	@Autowired
	OffreAgenceRepository2 offreRepo;
	@Autowired
	HotelRepository2 hotelRepo;
	@Autowired
	ClientRepository2 clientRepo;
	@Autowired
	RestTemplate proxy;

	private static final String uri = "agenceservice2/api";
	...
	public ArrayList<OffreAgence> consulterDesOffres(@RequestBody Map<String, String> json) throws InvalidDateException, NotFoundException, ParseException, InvalidPasswordException {
		...
		for (Hotel2 hotel : hotelRepo.findAll()) {
			System.out.println("hotel numero : "+ hotel.getIndexHotel());
	...
	public int faireUneReservation(@RequestBody Map<String, String> json) throws NotFoundException {
		...
		Hotel2 hotel = hotelRepo.findById(hotelId).get();
